...
def eval_genome_parallel(genomes, config):
...
def eval_genome_mp(genomes, config):
    env.generation_count += 1
    nets = [neat.nn.recurrent.RecurrentNetwork.create(genome, config)
            for genome_id, genome in genomes]

    pool = Pool(7)
    fit = pool.map(env.evaluate, nets)
    pool.close()
    pool.join()
    print(env.generation_count, fit, max(fit))

    save((genomes[fit.index(max(fit))][1], config), env.instance_no)
    for i in range(len(genomes)):
        genomes[i][1].fitness = fit[i]
...
if __name__ == '__main__':
    ...
    winner = p.run(eval_genome_parallel)
    # run_saved_genome(8504)
